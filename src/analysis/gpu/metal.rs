...
impl MetalPCG {
    pub fn new() -> Result<Self, String> {
        #[cfg(feature = "gpu")]
        {
            ...
            let pipeline_precondition = Self::create_pipeline(&device, &library, "precondition_kernel")?;
            let pipeline_vector_update = Self::create_pipeline(&device, &library, "vector_update_kernel")?;
            ...
            let pipeline_copy_scalar = Self::create_pipeline(&device, &library, "copy_scalar_kernel")?;
            let pipeline_zero_scalar = Self::create_pipeline(&device, &library, "zero_scalar_kernel")?;
            let pipeline_pcg_iteration = Self::create_pipeline(&device, &library, "pcg_iteration_kernel")?;
            let pipeline_pcg_update_vectors = Self::create_pipeline(&device, &library, "pcg_update_vectors_kernel")?;
            let pipeline_dot_to_buffer = Self::create_pipeline(&device, &library, "dot_product_to_buffer_kernel")?;
            let pipeline_compute_alpha_beta = Self::create_pipeline(&device, &library, "compute_alpha_beta_kernel")?;
    ...
    fn gpu_pcg_solve(
        ...
        for iter in 0..max_iterations {
            self.spmv_gpu(&buf_values, &buf_col_indices, &buf_row_offsets, &buf_p, &buf_ap, n)?;
    ...
    #[cfg(feature = "gpu")]
    fn axpy_gpu(&self, y: &Buffer, x: &Buffer, alpha: &Buffer, n: usize) -> Result<(), SolverError> {
        let buf_n = self.create_buffer(&[n as u32]);
    ...
    #[cfg(feature = "gpu")]
    fn precondition_gpu(&self, diag: &Buffer, r: &Buffer, z: &Buffer, n: usize) -> Result<(), SolverError> {
        let buf_n = self.create_buffer(&[n as u32]);
    ...
    #[cfg(feature = "gpu")]
    fn vector_update_gpu(&self, p: &Buffer, z: &Buffer, beta: &Buffer, n: usize) -> Result<(), SolverError> {
        let buf_n = self.create_buffer(&[n as u32]);
