use fe_engine::audit::trail::{AuditEntry, AuditTrail, AuditValue};
...
fn test_audit_trail_records_assembly_phase() {
    ...
    assert!(
        !assembly_entries.is_empty(),
        "Should record stiffness matrix assembly"
    );

    let load_entries = audit.filter_by_action("assemble_load_vector");
    assert!(
        !load_entries.is_empty(),
        "Should record load vector assembly"
    );
}
...
fn test_audit_trail_records_solver_details() {
    ...
    assert!(
        !solve_entries.is_empty(),
        "Should record linear system solve"
    );

    if let Some(entry) = solve_entries.first() {
        let has_matrix_size = entry
            .details
            .iter()
            .any(|(k, _)| k.contains("matrix_size") || k.contains("dofs"));
...
fn test_audit_trail_records_boundary_conditions() {
    ...
    assert!(
        !bc_entries.is_empty(),
        "Should record boundary condition application"
    );
}
...
fn test_audit_trail_includes_element_force_recovery() {
    ...
    assert!(
        !force_entries.is_empty(),
        "Should record element force computation"
    );
}
...
fn test_audit_trail_includes_reaction_calculation() {
    ...
    assert!(
        !reaction_entries.is_empty(),
        "Should record support reaction computation"
    );
}
...
fn test_audit_trail_is_chronological() {
    ...
    for i in 1..audit.entries.len() {
        let prev_time =
            chrono::DateTime::parse_from_rfc3339(&audit.entries[i - 1].timestamp).unwrap();
        let curr_time = chrono::DateTime::parse_from_rfc3339(&audit.entries[i].timestamp).unwrap();
        assert!(
            curr_time >= prev_time,
            "Audit entries should be chronological"
        );
    }
}
